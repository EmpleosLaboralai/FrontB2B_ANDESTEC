...
export class ChatMgComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  scrollToBottom(): void {
    try {
      ...
      console.log('error scrol', err);
    }
  }

  consultarTipoDispositivo() {
    this.eventMediator.deviceChanged.subscribe({
      next: (resp) => {
        console.log(resp);
        this.typeDevice = resp;
        if (this.typeDevice?.isMobile) {
          this.showPostulantes = true;
          this.showChat = false;
      ...
      complete: () => {
        console.log('complete eventMediator.deviceChanged.subscribe()');
      },
    });
  }
