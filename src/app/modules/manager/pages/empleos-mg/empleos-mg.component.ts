...
export class EmpleosMgComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy(): void {
    ...
    if (this.suscriptionListado) {
      this.suscriptionListado.unsubscribe();
    }
  }
  ...
  cargarEmpleos() {
    ...
    this.suscriptionListado = this.empresaService
      .listarEmpleosOpenClose(req)
      .subscribe({
        next: (resp) => {
  ...
  eliminarEmpleo() {
    ...
    this.suscriptionEli = this.empresaService
      .eliminarEmpleosPorIds(req)
      .subscribe({
        next: (resp) => {
          console.log(resp);
